...
def train_CNN_cifar10(input_images, input_labels,train_step, batch_size, num_step,x,y):
   """
   ...
   with tf.Session() as sess:
        ...
        for i in range(num_step):
            ...
            sess.run(train_step, feed_dict={x: train_img, y: train_lab, keepProb: 0.5})
            
            if i%1000 == 0: 
                valid_acu = sess.run(accuracy, {x: valid_img, y: valid_lab, keepProb:1.0})
                print("\rAfter step {0:3d}, validation accuracy {1:0.4f}".format(i, valid_acu))
            if i%10000 == 0:
                saver = tf.train.Saver()
                saver.save(sess, "./model_on_cifar10/", global_step=i)
...
def max_activations_CONV1_cifar10(input_images, input_labels, batch_size, n1):
    """
...
#     input_lab = batch_all[1]
    ...
    featu_Conv1 = tf.placeholder(tf.float32, [batch_size, 32, 32, n1],name='featu_Conv1')
    ...
    with tf.Session() as sess:
        ...
        images = sess.run(x, feed_dict={x: input_images})
        ...
        n_cols = 8
        n_rows = math.ceil(batch_size*2 / n_cols) + 1
...
def max_activations_CONV2_cifar10(input_images, input_labels, batch_size, n1, n2):
    """
...
def Grad_cifar10(layer_name, input_images, input_labels, batch_size):
    """
    ...
    with tf.Session() as sess:
        ...
        g_vals = sess.run(gradients, feed_dict={x: input_images})
        print("")
...
def getActivations(layer_name, input_images, image_idx):
    """
    ...
    for i in range(num_filters):
        ...
        plt.imshow(filter_units[image_idx, :, :, i], interpolation='nearest', cmap='rainbow')
